...
class VQADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        final_string = f"ObjRec:{obj_feature}<SEP>TextRec:{txt_feature}<SEP>ColorRec:{col_feature}<SEP>Count:{cnt_feature}<SEP>Answer:{highest_confidence_answer}<END>"
...
2. **Answer the question** directly in 1-3 words based on the image and caption. 
...
ObjRec: {1 or 0}<SEP>TextRec: {1 or 0}<SEP>ColorRec: {1 or 0}<SEP>Count: {1 or 0}<SEP>Answer: {Direct Answer in 1-3 Words}
...
### Example Output:
ObjRec:1<SEP>TextRec:1<SEP>ColorRec:1<SEP>Count:1<SEP>Answer:Red<END>
"""
