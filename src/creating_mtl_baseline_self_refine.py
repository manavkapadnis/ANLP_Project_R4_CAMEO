...
class MultiTaskVQAModelSelfRefine(pl.LightningModule):
    """
    ...
    def forward(self, samples):
        ...
        img_embeds, atts_img, pooled_img_rep = self.encode_img(image)
        img_embeds = self.layer_norm(img_embeds)
        pooled_img_rep = self.layer_norm(pooled_img_rep)
        ...
        gumbel_softmax_logits = F.gumbel_softmax(outputs.logits[:, empty_targets.shape[1]: , :], dim = 2, tau = 0.07, hard = False)
        response_embeddings = torch.matmul(gumbel_softmax_logits, self.llama_model.get_input_embeddings().weight)
        attention_weights = outputs.attentions[-1][:, :, empty_targets.shape[1]: , empty_targets.shape[1]:]
        
        pooled_response_rep = torch.bmm(attention_weights.mean(dim = 3), response_embeddings).mean(dim = 1)

        loss = nn.CrossEntropyLoss()(outputs.logits.view(-1, outputs.logits.size(-1)), targets.view(-1))
        
        refining_loss = torch.exp(
                torch.sum(
                    (torch.mul(pooled_response_rep, pooled_img_rep.squeeze()).sum(dim=1) / (torch.linalg.norm(pooled_img_rep.squeeze(), dim=1) * torch.linalg.norm(pooled_response_rep, dim=1))).squeeze(),
                    dim = 0
                )
            )
        ...
        return {"loss": 0.5 * loss + 0.5 * refining_loss}
...
def train(args):
    ...
    if args.ckpt_file is not None:
        model = MultiTaskVQAModelSelfRefine.load_from_checkpoint(args.ckpt_file, strict=False)
    else:
        model = MultiTaskVQAModelSelfRefine(args)
